...
const inputClassName =
  "mt-2 h-12 w-full rounded-xl border border-slate-200 bg-[#f8faf8] px-4 text-sm text-slate-900 outline-none transition-all placeholder:text-slate-400 focus:border-emerald-700 focus:bg-white focus:ring-3 focus:ring-emerald-700/10";
      ...
      {open && (
        ...
        >
          ...
          <motion.div
            ...
            className="relative z-10 flex max-h-[94svh] w-full max-w-[920px] flex-col overflow-hidden rounded-t-3xl bg-white shadow-[0_35px_100px_rgba(0,0,0,0.28)] sm:max-h-[90svh] sm:rounded-3xl lg:grid lg:grid-cols-[300px_1fr]"
          ...
          >
            <button type="button" onClick={closeAndReset} className="absolute right-4 top-4 z-20 grid h-9 w-9 place-items-center rounded-full border border-slate-200 bg-white text-slate-600 shadow-xs hover:bg-slate-100" aria-label="Close popup"><X className="h-4 w-4" /></button>

            <aside className="hidden bg-emerald-950 p-7 text-white lg:block">
              <span className="grid h-11 w-11 place-items-center rounded-2xl bg-white/10 text-emerald-300"><CalendarDays className="h-5 w-5" /></span>
            ...
            <div className="overflow-y-auto">
              ...
              <form onSubmit={handleSubmit} className="flex min-h-full flex-col p-5 sm:p-7 lg:p-9">
                ...
                {submitted && <div className="flex min-h-[390px] flex-col items-center justify-center text-center"><span className="grid h-16 w-16 place-items-center rounded-2xl bg-emerald-100 text-emerald-700"><CheckCircle2 className="h-8 w-8" /></span><h3 className="mt-6 font-montserrat text-3xl font-semibold text-emerald-950">Request is ready</h3><p className="mt-3 max-w-[390px] text-sm leading-6 text-slate-600">WhatsApp will open with your appointment details. Send the prepared message to complete your enquiry.</p><button type="button" onClick={closeAndReset} className="mt-7 rounded-full bg-emerald-950 px-7 py-2.5 text-sm font-medium text-white cursor-pointer">Close</button></div>}
                {!submitted && <div className="mt-9 flex items-center justify-between gap-3 border-t border-slate-100 pt-5">{step > 1 ? <button type="button" onClick={() => setStep((current) => current - 1)} className="inline-flex min-h-11 items-center gap-2 rounded-full px-5 text-sm text-slate-600 hover:bg-slate-100 cursor-pointer"><ArrowLeft className="h-4 w-4" />Back</button> : <span />}{step < 3 ? <button type="button" disabled={!canContinue} onClick={() => setStep((current) => current + 1)} className="inline-flex min-h-11 items-center gap-2 rounded-full bg-emerald-950 px-6 text-sm font-medium text-white disabled:cursor-not-allowed disabled:opacity-40 cursor-pointer">Continue<ArrowRight className="h-4 w-4" /></button> : <button type="submit" className="inline-flex min-h-11 items-center gap-2 rounded-full bg-[#25D366] px-6 text-sm font-semibold text-white shadow-xs hover:brightness-105 cursor-pointer"><MessageCircle className="h-4 w-4" />Send on WhatsApp<ArrowRight className="h-4 w-4" /></button>}</div>}
              </form>
            </div>
          </motion.div>
        </motion.div>
