...
export function Treatments({ onBook }: TreatmentsProps) {
  ...
  return (
    <section
      id="treatments"
      className="relative isolate overflow-hidden bg-[#fbfcfa] py-8 sm:py-12"
    >
      <div className="mx-auto w-full max-w-[1170px] px-4 sm:px-6 lg:px-8">
        {/* Compact Brief Header */}
        <div className="flex flex-col items-center text-center justify-between gap-4 border-b border-slate-200/80 pb-6 sm:flex-row sm:items-end sm:text-left">
          <div className="flex flex-col items-center sm:items-start">
            <div className="inline-flex items-center gap-2 rounded-full bg-emerald-50 px-3.5 py-1 text-[11px] font-bold uppercase tracking-[0.18em] text-emerald-900">
              <span className="h-1.5 w-1.5 rounded-full bg-emerald-600" />
              Specific Treatments
            </div>

            <h2 className="mt-2.5 font-montserrat text-[24px] font-semibold tracking-[-0.03em] text-emerald-950 sm:text-3xl text-center sm:text-left">
              Targeted clinical therapies
            </h2>

            <p className="mt-1 text-xs leading-relaxed text-slate-500 sm:text-sm text-center sm:text-left">
          ...
          <button
            ...
            className="group inline-flex shrink-0 items-center justify-center gap-2 rounded-full border border-slate-200 bg-white px-5 py-2 text-xs font-semibold text-emerald-950 shadow-2xs transition-colors hover:border-emerald-800 hover:bg-emerald-50 cursor-pointer"
          >
          ...
          {briefTreatments.map((item, idx) => {
            ...
            return (
              <motion.button
                ...
                className="group flex flex-col justify-between rounded-2xl border border-slate-200/80 bg-white p-4.5 text-left shadow-[0_2px_12px_rgba(15,23,42,0.03)] transition-all duration-300 hover:-translate-y-0.5 hover:border-emerald-800/40 hover:shadow-[0_8px_24px_rgba(6,78,59,0.08)] cursor-pointer"
              >
                <div className="flex items-start gap-3.5">
                  <span className="grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-emerald-50 text-emerald-800 transition-colors duration-300 group-hover:bg-emerald-950 group-hover:text-white">
